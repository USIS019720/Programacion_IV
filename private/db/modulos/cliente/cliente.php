...
class cliente{
    ...
    private function almacenar_datos(){
        ...
        }else if( $this->datos['accion']=='nuevo'){
            $this->db->consultas('INSERT INTO clientes(codigo, nombre, direccion, telefono, dui)
            VALUES(?,?,?,?,?)',
            $this->datos['codigo'],$this->datos['nombre'],$this->datos['direccion'],$this->datos['telefono'],$this->datos['dui']
        );
        return $this->db->obtenerUltimoid();
        }else if( $this->datos['accion']=='modificar'){
            $this->db->consultas('UPDATE clientes(codigo, nombre, direccion, telefono, dui)
            WHERE idCliente=?',
            $this->datos['codigo'],$this->datos['nombre'],$this->datos['direccion'],$this->datos['telefono'],$this->datos['dui'],$this->datos['idCliente']
        );
